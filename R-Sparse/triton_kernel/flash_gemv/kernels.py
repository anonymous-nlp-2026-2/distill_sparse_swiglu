...
@triton.jit
def gather_transposed_gemv_elemul_flag_kernel(
    Y,  # Pointers to matrices
    A,
    X,
    X_1,
...
def gather_transposed_gemv_elemul_flag_3d(
    x: torch.Tensor,
    x_1: torch.Tensor,
    wup: torch.Tensor,
    idx: torch.Tensor,
    ...
    gather_transposed_gemv_elemul_flag_kernel[grid](
        output,  # data ptrs
        wup,
        x,
        x_1,
...
@triton.jit
def gather_transposed_gemv_flag_atomicadd_kernel_silu(
    Y,  # Pointers to matrices
    A,
    X,
    IDX,
    ...
    if BATCHSIZE == 1:
        ...
        acc0 = apply_activation(acc0, ACTIVATION='swish')
    elif BATCHSIZE == 2:
        idx_1 = tl.load(IDX + M, mask=rm < M, other=0) > 0
        i_mask = (idx | idx_1)[:, None]
        a = tl.load(A, mask=i_mask, other=0.0).to(tl.float32)
        ...
        acc0 = apply_activation(acc0, ACTIVATION='swish')
        acc1 = apply_activation(acc1, ACTIVATION='swish')
        ...
        acc0 = apply_activation(acc0, ACTIVATION='swish')
        acc1 = apply_activation(acc1, ACTIVATION='swish')
        acc2 = apply_activation(acc2, ACTIVATION='swish')
        ...
        acc1 = apply_activation(acc1, ACTIVATION='swish')
        acc2 = apply_activation(acc2, ACTIVATION='swish')
        acc3 = apply_activation(acc3, ACTIVATION='swish')
    ...
    if BATCHSIZE >= 4:
        tl.atomic_add(Y + 3 * N, acc3, mask=rn < N)
def gather_transposed_gemv_flag_3d_activation(
    x: torch.Tensor,
    weight: torch.Tensor,
    idx: torch.Tensor,
) -> torch.Tensor:
    ...
    kernel = gather_transposed_gemv_flag_atomicadd_kernel_silu
    kernel[grid](
        output,  # data ptrs
        weight,
        x,
